...
import com.library.controller.PageQueryController;
import com.library.controller.ReaderTypeController;
...
public class UserBook extends JPanel implements ActionListener, ItemListener {
    ...
    protected JPanel addPanel0() throws SQLException {
        ...
        pageCount = new PageQueryController(bookcon.selectBook()).pageCount();
        ...
        // 设置表格模型的数据
        dfttable = new DefaultTableModel(new PageQueryController(bookcon.selectBook()).setCurrentPageIndex(), columnName);
        ...
        // 设置表格的公共属性
        TableUtils.setTable(table, dfttable);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == jbt) {
            if (b_type == "*" || b_type == null) {
                try {
                    if (jtext.getText().equals("") || jtext.getText().equals("ISBN/书名/作者")) {
                        setTableMolel(new PageQueryController(bookcon.selectBook()).setCurrentPageIndex());
                    } else {
            ...
            if (b_id != -1) {
                ...
                if (c == JOptionPane.YES_OPTION) {
                    try {
                        for (int i = 0; i < readerTypeCon.queryPersonalType(UserView.count).size(); i++) {
                            // 根据账号查询出读者类型的每本书的可借阅天数
                            maxday = Integer.valueOf(readerTypeCon.queryPersonalType(UserView.count).elementAt(i)
                                    .elementAt(2).toString());
                            // 同理，得可借阅数量
                            maxcount = Integer.valueOf(readerTypeCon.queryPersonalType(UserView.count).elementAt(i)
                                    .elementAt(1).toString());
                        }
                        // 已借阅的图书数量
                        borrowingCount = borrowcon.queryBorrowInfo(UserView.count, UserView.count, false).length;
                        int borrowdate = DateUtils.getNewStamp();
                        int duedate = DateUtils.getNewStamp() + maxday * 86400;// 1天86400秒
                        if (borrowingCount <= maxcount) {
                            if (borrowcon.queryIsBorrowBook(b_id, UserView.count) == false) {
                                // 插入借阅时间和应该归还时间，并将图书对应的库存量-1（事务）
                                if (borrowcon.insertBorrow(UserView.count, b_id, borrowdate, duedate, b_id)) {
            ...
            try {
                setTableMolel(new PageQueryController(bookcon.selectBook()).previousPage());
                if (pageIndex > 1) {
                    pageIndex--;
                    jlab_book.setText("第" + pageIndex + "页/" + "共" + pageCount + "页");
                }
            ...
            try {
                setTableMolel(new PageQueryController(bookcon.selectBook()).nextPage());
            ...
            try {
                setTableMolel(new PageQueryController(bookcon.selectBook()).lastPage());
                pageIndex = pageCount;
                jlab_book.setText("第" + pageIndex + "页/" + "共" + pageCount + "页");
            } catch (SQLException e1) {
                // TODO Auto-generated catch block
            ...
            try {
                ...
                if (isInt.length() == 0) {
                    if (Integer.valueOf(jtext_page.getText()) > 0
                        ...
                        setTableMolel(
                                new PageQueryController(bookcon.selectBook()).jumpPage(Integer.valueOf(jtext_page.getText())));
         ...
         */
        b_id = TableUtils.cancelTableSelected(table, b_id);
        // 尾页时，“下一页”和“尾页”按钮不可用
        if (pageIndex == pageCount) {
