...
public class ManageReader {
    ...
    protected JPanel addPanel2() throws SQLException {
        ...
        jtext_reader.addFocusListener(new InputLimit(jtext_reader, "借阅证号/姓名/院系/班级"));//
        // 设置文诓提示的外部类监听
        ...
        String[] columnReader = { "借阅证号", "姓名", "性别", "读者类型", "院系", "班级", "手机号码", "电子邮箱" };
        for (int k = 0; k < columnReader.length; k++) {
            columnNameReader.add(columnReader[k]);
        }
        DefaultTableModel dfttable_reader = new DefaultTableModel(readerCon.selectReader(), columnNameReader);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    if (readerTypeInfo == "*" || readerTypeInfo == null) {
                        if (jtext_reader.getText().equals("借阅证号/姓名/院系/班级") || jtext_reader.getText().equals("")) {
                            dfttable_reader.setDataVector(readerCon.selectReader(), columnNameReader);
                        } else {
                        ...
                        if (jtext_reader.getText().equals("借阅证号/姓名/院系/班级")) {
                            jtext_reader.setText("");
                        }
                        dfttable_reader.setDataVector(
                                readerCon.selectReaderInfo(jtext_reader.getText(), jtext_reader.getText(),
                ...
                studentNumber = TableUtils.setNull(table_reader, studentNumber);
            }
        });
        // 新增
