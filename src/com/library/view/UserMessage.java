...
public class UserMessage {
    ...
    protected JPanel addPanel2() throws SQLException {
        ...
        Vector<Vector<Object>> readerInfo = readercon.queryReaderInfo(UserView.count);
        String[] columnReader = { "借阅证号", "姓名", "性别", "院系", "班级", "手机号码", "电子邮箱" };
        ...
        DefaultTableModel dfttable_readerType = new DefaultTableModel(readerTypeCon.queryPersonalType(UserView.count),
        ...
        // 设置表格的公共属性
        TableUtils.setTable(table_reader, dfttable_reader);
        TableUtils.setTable(table_readerType, dfttable_readerType);
        ...
        alterX.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                JLabel[] jlab_reader = { new JLabel("借阅证号："), new JLabel("院系："), new JLabel("班级："), new JLabel("手机号："),
                        new JLabel("邮箱：") };
                ...
                try {
                    Object[] readerUpdata = { UserView.count,
                            readercon.queryReaderInfo(UserView.count).elementAt(0).elementAt(3),
                            readercon.queryReaderInfo(UserView.count).elementAt(0).elementAt(4),
                            readercon.queryReaderInfo(UserView.count).elementAt(0).elementAt(5),
                            readercon.queryReaderInfo(UserView.count).elementAt(0).elementAt(6) };
        ...
        alterM.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                new PubJdialog(UserView.count, true).setVisible(true);// 用对话框进行修改密码
            }
        });
        return panel;
    }
