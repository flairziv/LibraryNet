...
public class LoginView extends JFrame implements ActionListener, ItemListener {
    JLabel[] jlab = {new JLabel("借阅证号："), new JLabel("姓名："), new JLabel("性别："), new JLabel("读者类型："), new JLabel("院系："),
    ...
    public void actionPerformed(ActionEvent e) {
        String[] regex = {InputLimit.STUDENT_NUMBER_REGEX, InputLimit.NAME_REGEX, InputLimit.CHINESE_REGEX, InputLimit.CHINESE_MATH_REGEX,
                InputLimit.TELEPHONE_REGEX, InputLimit.EMAIL_REGEX, InputLimit.PASSWORD_REGEX};// 验证借阅证号（以20开头的11
        ...
        String[] hintError = {"借阅证号格式错误", "姓名格式错误", "院系格式出错", "班级格式出错", "手机号格式错误", "邮箱格式错误", "密码格式错误（6-16位数字或字母）"};
        String message = "";
            ...
            try {
                ...
                if (message.equals("")) {// 进行正则验证
                    ...
                    if (new String(jpassword[0].getPassword()).equals(new String(jpassword[1].getPassword()))) {
                        // 检查账号是否被注册
                        if (! readercon.isCard_number(jtext[0].getText())) {
                            readercon.insertReader(jtext[0].getText(), jtext[1].getText(), gender,
                                    ...
                                    MD5Utils.string2MD5(new String(jpassword[0].getPassword())));
                            // 将密码进行MD5加密后在添加信息
